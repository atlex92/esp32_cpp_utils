...
GpioButton::GpioButton(const gpio_num_t gpio, const eGpioButtonPressedLogicLevels pressed_logic_level,
                       const bool pullup, const uint32_t debounce_period_ms)
: AbstractButton(debounce_period_ms), gpio_(gpio), pressed_logic_level_(pressed_logic_level), pullup_(pullup) {
...
eButtonStates GpioButton::readState() const {
  ...
  if (eGpioButtonPressedLogicLevels::BUTTON_PRESSED_LEVEL_HIGH == pressed_logic_level_) {
    return GpioUtils::eGpioOutputValues::GPIO_HIGH == gpio_level ? eButtonStates::BUTTON_PRESSED
                                                                 : eButtonStates::BUTTON_RELEASED;
  } else {
    return GpioUtils::eGpioOutputValues::GPIO_HIGH == gpio_level ? eButtonStates::BUTTON_RELEASED
                                                                 : eButtonStates::BUTTON_PRESSED;
  }
}