...
class NtcSensor {
public:
  NtcSensor(const NtcCfg& cfg) : cfg_{cfg} {
  }
...
private:
  ...
  */
  static float ntcResistanceToTemperatue(const uint32_t resistance, const uint32_t nominal_resistance,
                                         const uint32_t b_coeff, const float nominal_temp) {
    if (0 == resistance) {
