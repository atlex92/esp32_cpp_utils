...
struct PwmFanGroupCfg {
  std::vector<gpio_num_t> cntrl_gpios;
...
class PwmFanGroup {
public:
  explicit PwmFanGroup(const PwmFanGroupCfg& cfg) : cfg_{cfg} {
    assert(cfg.cntrl_gpios.size() > 0U);
    initialize();
  }
...
private:
  ...
  void initLedChannel() {
    ledc_channel_config_t ledc_channel_cfg{.gpio_num = {},
    ...
    for (size_t i{}; i < cfg_.cntrl_gpios.size(); i++) {
      ledc_channel_cfg.gpio_num = cfg_.cntrl_gpios[i];
