...
public:
  explicit AdcChannel(const adc_unit_t unit, const adc_channel_t channel, const adc_bitwidth_t bitwidth,
                      const adc_atten_t attenuation)
  : unit_{unit}, channel_{channel}, bitwidth_{bitwidth}, attenuation_{attenuation} {
  }
  ...
  bool initializeImpl() override {
    {
      ...
      if (!adc_handle_) {
        const adc_oneshot_unit_init_cfg_t adc_cfg{unit_, ADC_RTC_CLK_SRC_RC_FAST, ADC_ULP_MODE_DISABLE};
        ESP_ERROR_CHECK(adc_oneshot_new_unit(&adc_cfg, &adc_handle_));
      }
    }

    const adc_atten_t attenuations[4]{ADC_ATTEN_DB_0, ADC_ATTEN_DB_2_5, ADC_ATTEN_DB_6, ADC_ATTEN_DB_12};

#if ADC_CALI_SCHEME_CURVE_FITTING_SUPPORTED
    adc_cali_curve_fitting_config_t cali_cfg{unit_, channel_, ADC_ATTEN_DB_0, bitwidth_};
    for (size_t i{}; i < 4; i++) {
      cali_cfg.atten = attenuations[i];
      ESP_ERROR_CHECK(adc_cali_create_scheme_curve_fitting(&cali_cfg, &adc_cali_[i]));
    }
#else
    adc_cali_line_fitting_config_t cali_cfg{unit_, ADC_ATTEN_DB_0, bitwidth_};
    for (size_t i{}; i < 4; i++) {
      cali_cfg.atten = attenuations[i];
