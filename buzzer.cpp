...
void Buzzer::proccessMelody() {
    ...
    const uint32_t freq{getNoteFrequency(current_melody_.sounds[current_note].note, current_melody_.sounds[current_note].octave)};
...
void Buzzer::addMelodyToQueue(const Melody& melody) {
    ...
    auto duplicate_melody_lambda = std::find_if(melodies_.begin(), melodies_.end(), [&melody](const Melody& inner_melody) 
        {
            return (strcmp(melody.tag,  inner_melody.tag) == 0);
        }
    );
...
void Buzzer::deleteMelodyFromQueue(const char* const tag) {
    auto remove_melodies_lambda = [&] (const Melody& melody) -> bool {
        return (0 == strcmp(tag, melody.tag));
    };
