...
void Uart::uartEventTask(void* args) {
  ...
  auto idleDetectionTask = [&uart]() {
    while (uart->_lastTimeDataReceived and
           false == TimeUtils::isPeriodPassedMs(uart->_lastTimeDataReceived, UART_IDLE_PERIOD_MS)) {
    ...
    if (xQueueReceive(uart->_eventQueue, (void*) &event, (TickType_t) portMAX_DELAY)) {
      ...
      switch (event.type) {
        ...
        case UART_BUFFER_FULL:
          ...
          // //     if (pos == -1) {
          // //         // There used to be a UART_PATTERN_DET event, but the pattern position queue is full so that it
          // can not
